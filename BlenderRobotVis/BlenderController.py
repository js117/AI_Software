...
try:
	from msvcrt import getch, kbhit  # try to import Windows version
except ImportError:
...
#filename = os.path.join(os.path.basename(bpy.data.filepath), "BlenderController.py")

fileprefix = os.path.dirname(os.path.abspath(__file__))

blendname = bpy.path.basename(bpy.context.blend_data.filepath)

filename = "BlenderController.py"

final_filepath = fileprefix.replace(blendname, filename)
print("------------")
print(fileprefix)
print(blendname)
...
exec(compile(open(final_filepath).read(), final_filepath, 'exec'))
''' 
